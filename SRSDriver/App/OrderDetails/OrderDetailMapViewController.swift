...
extension OrderDetailMapViewController {
  ...
  func checkLocationStatus() {
    guard let _ = Cache.shared.getObject(forKey: Defaultkey.firstLaunch) as? Bool else {
      Cache.shared.setObject(obj: true, forKey: Defaultkey.firstLaunch)
      return
    }
    guard CLLocationManager.authorizationStatus() == .notDetermined || CLLocationManager.authorizationStatus() == .denied else {return}
    showAlertView("Pleae allow accessing your location") { (ok) in
